...
| `--reflection-model openai/gpt-5.6-terra`   | Deliberately not `--model`: a 4B model writing its own instructions is the weakest link in the loop. The `openai/` prefix is LiteLLM routing, stripped before the gateway sees it.                                                                                |
...
| `--reflection-model` | `openai/gpt-5.6-terra`       | Writes the instructions during `--optimizer gepa`. Keep it stronger than `--model`.                    |
