  ...
  // `cancelDictation` directly — the same methods the key tap's closures
  // dispatch to — so a UI test drives the *same* pipeline (`DictationSession`)
  // and missing-key gate the hotkey would. Compiled only in Debug, so nothing
  // here ships in the notarized Release build.
