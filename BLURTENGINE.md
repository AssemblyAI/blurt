...
- **`KeyTermsStore`** persists the user's domain vocabulary (names, jargon) in `UserDefaults`; `DictationSession` re-reads it at every press via its `keyTermsProvider` closure, so Settings edits apply to the next utterance without rebuilding the session. Pass your own provider to source terms from elsewhere. Their old `Keywords:` clause in the transcription prompt is gone, so today they reach nothing.
...
- **No English-pinning or filler-word clauses in the prompt**, and no widening it past the prior chunk — the app name, window title, field label and selected text stay on the machine.
