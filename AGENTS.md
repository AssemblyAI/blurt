...
| Layer                  | What it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Sources/BlurtEngine/` | Swift package (`swift-tools-version:6.2`, `platforms: [.macOS(.v15), .iOS(.v18)]`) owning the pipeline. Pure logic behind protocol seams, no AppKit-shell deps, **no external SPM deps** — Foundation/Security/AVFoundation/CoreAudio plus toolchain modules like Synchronization, with AppKit types only at the seams. The macOS-only files (Accessibility reads, clipboard paste, CoreAudio route reads, TCC and code-signing helpers) are fenced behind `#if os(macOS)`; `check.yml`'s `ios-build` job builds the library for the iOS simulator so a symbol that escapes its fence fails the PR. |
...
| `App/BlurtiOS/`        | The iPhone app (SwiftUI) and the keyboard it embeds, generated by XcodeGen from its own `project.yml` and built by `check.yml`'s `ios-build` job. The app owns the engine's pipeline over an always-on capture; the keyboard is a remote control that inserts the words. `App/BlurtiOS/README.md` is its guide.                                                                                                                                                                                                                                                                                     |
...
Sources/BlurtEngine/         the engine (dependency-free Swift package)
  ...
  Audio/                     MicCapture (+meter) over CaptureSessionRecorder (the
                             ...
                             AudioRoute (+Monitor) — the CoreAudio output route (macOS only),
                             AudioTransport — what a transport type means,
  ...
  Config/                    Keychain-backed API key, key terms, developer mode, style profiles,
                             ...
                             sweep over them)
  FocusCapture/              FocusContext (CapturedFocus + FocusedFieldContext, every platform);
                             the Accessibility reads of the frontmost app / focused field
                             (macOS only); +Pure, the decisions that need no AX call
  Hotkey/                    TriggerKey(+Store), DictationKeyGate, DictationKeyRouter
  Injection/                 InsertionSeparator (the spacing rule, every platform); KeyInjector
                             (clipboard paste) + SystemClipboard (macOS only)
  Permissions/               PermissionStatus + SetupReadiness (every platform);
...
App/BlurtiOS/                the iPhone app and its keyboard (README.md there is the guide):
  project.yml                XcodeGen source of truth — the project is generated, NOT committed
  BlurtiOS/Sources/          the app: DictationCoordinator (composition root), ListeningWindow +
                             ...
                             the mic from the background), KeyboardRelayInjector, HomeView
  BlurtKeyboard/Sources/     the keyboard: a remote control that inserts the words; three
                             layouts (slim bar / panel / full keyboard) over one model
...
.github/workflows/           check.yml (the gate, macos-26; its ios-build job compiles
                             the engine for the iOS simulator),
...
for the host, and every other job runs on a Mac. `gate` fails unless both platform builds are green
(or skipped for a docs-only change). Two sibling jobs exist purely to shorten the loop for whoever is
...
leading separator space via `InsertionSeparator.withLeadingSeparator` (its own platform-free type,
because a host that inserts text itself faces the same decision); when the prior text is unreadable,
the separator basis falls back to what was last pasted — but only when the target app **and** window title both
