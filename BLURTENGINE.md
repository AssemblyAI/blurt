...
- Two ready-made projections keep UI mapping out of your shell: `phase.overlayState` (`OverlayUIState`: idle / recording / processing / error(message:) / pasted / noTarget, with accessibility labels and a transient-notice flag) and `phase.menuBarStatus` (coarser: idle / recording / transcribing, never shows errors).
