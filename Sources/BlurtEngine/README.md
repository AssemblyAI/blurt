...
- **`TriggerKey`** — the curated lone modifiers usable as a trigger (right ⌘, right ⌥, fn), with keycodes, display labels, and the modifier masks the event source needs (the per-side `NX_DEVICE*` bit for the two right-side keys, the shared `kCGEventFlagMaskSecondaryFn` for `fn`).
