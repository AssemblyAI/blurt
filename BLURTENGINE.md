...
- Production implementations of all three seams (`MicCapture`, `AssemblyAITranscriber`, `KeyInjector`), plus the supporting pieces a dictation product needs: Keychain-backed API-key storage, per-utterance contextual prompting (built and tested, currently switched off), a hotkey state machine, permission checks, and UI-state projections.
...
**The transcription prompt is currently switched off** (`TranscriptionPrompt.isEnabled == false`), so none of what follows reaches AssemblyAI: `build(context:)` returns `nil`, `config.prompt` is omitted, and the request is the audio plus the `llm` cleanup instruction. The context is still captured — it decides paste spacing and feeds the developer-mode log — and the builder is still tested, so turning priming back on is that one constant. Hosts don't call these APIs directly, but should know what's collected and what it's for:

- **`TranscriptionContext`** carries the frontmost app name, window title, focused-field label, the text before the caret, the selected text (which a paste will replace), and the user's key terms. It's captured via Accessibility at press time (skipped in secure fields), off the hot path — and consumed at transcribe time with a bounded wait (`DictationSession.contextWaitBudget`), so a hung read is abandoned rather than stalling the transcript. With the prompt off it stays on the machine: the injector reads `priorText`/`windowTitle` to decide the paste's leading separator, and the developer-mode log records the rest.
- **`TranscriptionPrompt.assemble(context:)`** renders that into what would be sent as the dictation request's `config.prompt` (`build(context:)` is the gate in front of it, and yields `nil` while the prompt is off) (transcription steering only — the cleanup rewrite is the separate `llm` block), opening with the fixed `baseInstruction` ("Transcribe without speaker labels, audio event descriptions, or emotion markers.") and staying under the API's 4096-character cap. An empty context yields `nil`, which omits the field so the server applies its own default. Two deliberate omissions, both regression-tested: no language directive (pinning to English hurt non-English speech) and no "remove filler words" clause (not in the model's trained instruction set — a no-op). Don't reintroduce either.
- **`CleanupInstruction.text`** is the other half: the string sent as `config.llm.instruction`, which the service applies to the verbatim transcript with its own rewrite model. It is the winner of a GEPA run of `evals/dictation-prompt/`, compressed to fit the API's **2048-character** cap on that field — a different, smaller limit than the 4096 on `config.prompt`, and one that rejects the entire request rather than degrading, so `CleanupInstruction.characterCap` is asserted in tests. Treat it as a measured artifact: a new hypothesis belongs in that harness plus a fresh run, not a hand-edit. Two caveats ship with it — it has only ever been scored against other text candidates on a stand-in model, never against the empty `llm` block on the service's own rewrite model, and every corpus behind it is English. Dropping the field restores the service default in one line.
- **`KeyTermsStore`** persists the user's domain vocabulary (names, jargon) in `UserDefaults`; `DictationSession` re-reads it at every press via its `keyTermsProvider` closure, so Settings edits apply to the next utterance without rebuilding the session. Pass your own provider to source terms from elsewhere. The terms reach the prompt builder and nowhere else, so while the prompt is off they have no effect on transcription.
...
The package builds and runs in any macOS 15+ host, but a handful of constants name Blurt specifically, and the ones that aren't injectable are worth knowing before you ship the engine inside another app:

| Constant                                      | Value                                                            | Overridable from outside                                                           |
| --------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `BlurtIdentity.keychainService`               | `blurt` — the API key item's Keychain service                    | No. Compose against `APIKeyGateway` with your own conformance instead.             |
| `BlurtIdentity.subsystem`                     | `dev.alex.blurt` — the unified-logging subsystem, every category | No.                                                                                |
| `DictationLog.defaultURL` / `defaultErrorURL` | `~/Library/Logs/Blurt/{dictations,errors}.jsonl`                 | No. The `to url:` parameter on the writers is internal, for tests.                 |
| `DefaultsKey` raw values                      | `Blurt`-prefixed keys, written into the host's defaults domain   | Key names, no; which `UserDefaults` they land in, yes (see the store table above). |
| `UpdateChecker`'s default `releaseURL`        | `AssemblyAI/blurt`'s latest release, and a `Blurt.dmg` asset     | Yes — an initializer parameter.                                                    |
| `UpdateAlertContent`'s wording                | "Blurt" as the product name                                      | No.                                                                                |

The cue audio is app-side rather than package-side for the same reason (see [Record cues](#record-cues)). None of this blocks embedding — the dictation pipeline itself is clean — but a second app on the same machine shares Blurt's Keychain item, log directory, and defaults keys unless you take the seams above.
