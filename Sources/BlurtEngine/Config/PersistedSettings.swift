...
/// the cleaned and raw trigger keys, sound pack, key terms, developer mode,
/// cleanup prompt, overlay origin, and the timestamp throttling the automatic
/// update check. Owned here — next to the stores — so adding a store and adding
/// it to every "reset to a clean state" sweep (e.g. the app's UI-test launch
/// reset) are the same edit, instead of a hand-maintained list in the app shell
/// that goes stale.
