...
LLM rewrite — the `config.llm` block Blurt sends on every `/v1/transcribe/live` request.
...
| `live.py`                    | Synthesis + the real `/v1/transcribe/live` round trip, for `--verify-live`.                   |
