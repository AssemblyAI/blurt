...
- Pill geometry is available too, if you're drawing something like Blurt's overlay: `OverlayPlacement` resolves where the panel goes (clearance, clamping a dragged origin back on screen), and `MeterBarGeometry` gives the level meter its shape — `barCount(availableWidth:)`, `maxBarHeight(availableHeight:)`, `barHeight(index:count:maxBarHeight:level:time:animated:)` for the bar row, and `breathingOpacity(time:period:minOpacity:)` for the pulse the record dot and status label share. All pure math; pass `animated: false` to honor Reduce Motion.
