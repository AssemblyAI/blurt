...
- **No streaming STT** — the AssemblyAI dictation API returns the full (already rewritten) text in one response, so the overlay shows "Transcribing…" then jumps to the full text.
...
- **No client-side LLM cleanup pass** — cleanup is the dictation API's server-side rewrite, requested by the `llm` block that rides on the same `/transcribe` call. There is no LLM Gateway client, no `StylerProtocol`, no post-transcription styling stage, and no second request. Don't reintroduce one; the rewrite instruction is server-owned (the empty `llm` block selects it), and transcription steering belongs in `TranscriptionPrompt`.
