...
- **`TranscriptionPrompt.build(context:)`** renders the prior chunk into the dictation request's `config.prompt` (transcription steering only — the cleanup rewrite is the separate `llm` block): the text before the caret under a `Previous transcript:` heading, closed by the fixed `baseInstruction` ("Transcribe without speaker labels, audio event descriptions, or emotion markers."), clipped to keep the tail under the API's 4096-character cap. It reads no other field of the context. A context with no prior text yields `nil`, which omits the field so the server applies its own default. Two deliberate omissions, both regression-tested: no language directive (pinning to English hurt non-English speech) and no "remove filler words" clause (not in the model's trained instruction set — a no-op). Don't reintroduce either, and don't widen the prompt back out to the app/window/field hints it used to carry.
...
- **`KeytermsBoost.fitted(_:)`** turns those terms into the request's `config.keyterms_prompt` — word boosting, the other steering field next to `prompt`, and a flat list of strings rather than prose. It takes whole terms in the user's order while they fit the field's own **2048-character** cap (`KeytermsBoost.characterCap`, in UTF-8 bytes — a third limit, distinct from the 4096 on `config.prompt` and the 2048 on `config.llm.instruction`), and returns a plain `[String]`; an empty result omits the field on the wire rather than sending `[]`. It is deliberately not the deprecated `word_boost`, which the Universal-3 Pro family rejects outright — that would fail the request, not degrade it. Don't fold key terms back into the prompt as a `Keywords:` clause; that was the older, worse shape for the same intent.
...
- **No English-pinning or filler-word clauses in the prompt**, and no widening it past the prior chunk — the app name, window title, field label and selected text stay on the machine. Key terms go on `keyterms_prompt`, never back into the prompt and never as the deprecated `word_boost`.
