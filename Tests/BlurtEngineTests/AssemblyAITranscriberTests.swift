...
struct HTTPClientTests {
  ...
  private func configObject(
    prompt: String?, keyterms: [String] = [], enhancedTranscripts: Bool = true, customStyle: String? = nil
  ) throws -> [String: Any] {
...
/// The word-boost half of the `config` contract (`KeytermsBoost` → the request's
/// `keyterms_prompt`). An extension of the same suite, exactly as the
/// `APIKeyValidator` cases are: it keeps the `configObject`/`makeTranscriber`
/// helpers and the `FakeHTTPTransport` seam, while leaving the suite's own body
/// inside SwiftLint's type-length limit.
extension HTTPClientTests {
  ...
  func configOmitsKeyterms() throws {
    ...
    // nothing to boost, so `DictationConfig.encode(to:)` drops the key. One
    // empty state to test, because `KeytermsBoost.fitted` returns a plain array.
    #expect(try configObject(prompt: nil, keyterms: []).keys.contains("keyterms_prompt") == false)
  }
}
