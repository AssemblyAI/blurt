...
- **`Sources/BlurtEngine/`** — Swift Package (`swift-tools-version:6.2`, macOS 15+) owning the pipeline. Pure logic behind protocol seams, no AppKit-shell dependencies and **no external SPM dependencies** (just Foundation/Security/AVFoundation plus toolchain modules like Synchronization).
...
This is a macOS-only project (`platforms: [.macOS(.v15)]`, AppKit + AVFoundation). On Linux or
...
harness (`App/Blurt/Blurt/UITestSupport.swift`, all `#if UITEST_HOOKS` — defined for the
Debug config, so `scripts/dev-build.sh` can strip it) that:
