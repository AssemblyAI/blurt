...
The engine ships the _decision logic_ for a single-lone-key trigger — a lone modifier, a lone function key, or an extra mouse button; the host supplies the event source (in Blurt, a `CGEventTap` — see `App/Blurt/Blurt/Hotkey/DictationKeyTap.swift` for the reference wiring).
...
- **`TriggerBinding`** — what's bound: `.modifier(TriggerKey)`, `.key(code:)` (F1–F20), or `.mouseButton(_:)` (button 2, "Mouse 3", and up — never the left/right click). All three encode into the store's single `Int` slot (modifiers as their bare keycode, so pre-Custom installs decode unchanged; F-keys raw; mouse buttons offset into a disjoint namespace); `fromPersisted(_:)` is the one decode-with-default rule (garbage → right ⌘), and `keyBinding(forKeyCode:)` / `mouseButtonBinding(forButton:)` are the capture-time policy for what a recorder may accept.
...
- **`DictationKeyRouter`** — the recommended layer over the gate: reduce each raw event to `.flagsChanged(keyCode:triggerFlagIsOn:)` / `.keyDown(keyCode:)` / `.keyUp(keyCode:)` / `.mouseDown(button:)` / `.mouseUp(button:)` and `handle(_:at:)` applies the filters every event source needs — only events for the bound trigger's own family count (`flagsChanged` for a modifier binding, key events for an F-key, mouse events for a button), and only genuine down/up _edges_ reach the gate (`flagsChanged` deliveries re-report the bit whether or not it changed, and a held key autorepeats, so a repeat must not double-start a dictation). The other-key combo cancel applies to modifier bindings only — under a key or mouse binding, other keys are just typing. `reset()` / `rebind(binding:)` clear state that can no longer be trusted (dropped events, a rebound trigger) and return whether they discarded a live recording. Unit-tested (`DictationKeyRouterTests`).
