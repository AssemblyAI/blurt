...
`CueReprimeGate` is its companion, and it exists because a pre-rolled player is only pre-rolled against the output route it was built against — see `AudioRouteMonitor`, whose ticks are the invalidation signal. Feed it `routeChanged()` per tick and `phaseChanged(to:)` per phase (after playing that phase's cue) and it answers one question: reload the players now, or hold the reload until the dictation ends? It re-primes immediately while idle and during `.connecting`, where nothing is waiting on the players and the chime that needs them is still ahead, and holds from `.recording` onwards, where a swap could displace a player mid-sound. Holding _every_ reload until the dictation ended — the obvious reading, and what Blurt shipped — leaves the session's first chime as the only one played through a stale pre-roll, which on a Bluetooth output route is audibly clipped.
