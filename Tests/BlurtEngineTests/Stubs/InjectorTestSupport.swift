  ...
  /// That application as the `CapturedFocus` `KeyInjector.setTarget` takes, so the
  /// insert suites pin a target the way `DictationSession` does.
  func liveTargetFocus() throws -> CapturedFocus {
    let app = try liveTargetApp()
